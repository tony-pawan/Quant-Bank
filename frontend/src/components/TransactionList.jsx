  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="bg-white/90 dark:bg-slate-900/90 border border-slate-200/60 dark:border-slate-800/60 rounded-3xl p-4 flex flex-col sm:flex-row gap-3 items-center justify-between shadow-sm">
        ...
        {/* Filters and Sorting */}
        <div className="flex flex-wrap w-full sm:w-auto items-center justify-end gap-2.5">
          {/* Type Filter */}
          <div className="flex items-center gap-1 w-[48%] sm:w-auto">
            ...
            <select
              ...
              className="w-full sm:w-auto text-[10px] font-extrabold bg-slate-50 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-750 text-slate-650 dark:text-slate-400 rounded-xl px-2 py-1.5 focus:outline-none cursor-pointer"
            >
          ...
          {/* Sort Control */}
          <div className="flex items-center gap-1 w-[48%] sm:w-auto">
            <span className="text-[9px] text-slate-400 dark:text-slate-500 font-extrabold uppercase tracking-wider hidden sm:inline">Sort:</span>
            <select
              value={sortOrder}
            ...
            >
              ...
              <option value="highest">Highest Amt</option>
              <option value="lowest">Lowest Amt</option>
            </select>
          </div>
          ...
          <div className="flex items-center gap-1 w-full sm:w-auto mt-1 sm:mt-0">
            ...
            <select
              ...
              className="w-full sm:w-auto text-[10px] font-extrabold bg-slate-50 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-750 text-slate-655 dark:text-slate-400 rounded-xl px-2.5 py-1.5 focus:outline-none cursor-pointer"
            >
      ...
      <div className="flex flex-col gap-3 max-h-[500px] overflow-y-auto pr-1">
        {transactions.length === 0 ? (
          <div className="bg-white dark:bg-slate-900 border border-dashed border-slate-200/80 dark:border-slate-800/80 rounded-3xl p-12 flex flex-col items-center justify-center text-center shadow-sm">
            <Inbox size={36} className="text-slate-350 dark:text-slate-600 mb-3" />
            <span className="font-extrabold text-slate-700 dark:text-slate-300 text-xs uppercase tracking-wider">No matching records</span>
            <p className="text-[11px] text-slate-400 dark:text-slate-550 mt-1 max-w-xs leading-relaxed">
              No matching records found. Try changing your search or filters.
            </p>
          </div>
        ) : (
          transactions.map(tx => (
